// Vendored from tauri-agent-tools (MIT, cesarandreslopez) — keep in sync
// with the package's examples/tauri-bridge/src/dev_bridge.rs so the
// README's "re-copy from the latest package" stays valid; the unused
// sidecar/diagnostics surface is allowed rather than trimmed.
#![allow(
    dead_code,
